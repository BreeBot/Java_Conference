...
public class Conference {
  ...
  public boolean register(Person newAttendee) {
    ...
    for (Person person : this.getAttendees()) {
      if(person.getEmail() == newAttendee.getEmail()) {
        return false;
      }
    }
  ...
  public String getSummary() {
    ...
    for(Session session : this.sessions) {
      sessionList += session.getName() + ", facilitated by " + session.getFacilitator().getFirstName() + " " + session.getFacilitator().getLastName() + "\n";
    }

    return this.name + "\n" + "Number of Attendees: " + this.attendees.size() + "===Attendees===" + attendeeList + "Number of Sessions: " + this.sessions.size() + sessionList;
